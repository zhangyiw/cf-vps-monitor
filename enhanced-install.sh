...
change_config() {
    ...
    echo -e "  1. API密钥: ${API_KEY}"
    echo -e "  2. 服务器ID: ${SERVER_ID}"
    echo -e "  3. Worker URL: ${WORKER_URL}"
    echo -e "  4. 返回主菜单"
    echo ""
    
    read -p "请选择要修改的配置项 [1-4]: " config_choice
    
    case $config_choice in
        1)
            read -p "请输入新的API密钥: " new_api_key
            if [ -n "$new_api_key" ]; then
                API_KEY="$new_api_key"
                echo -e "${GREEN}API密钥已更新${NC}"
            fi
            ;;
        2)
            read -p "请输入新的服务器ID: " new_server_id
            if [ -n "$new_server_id" ]; then
                SERVER_ID="$new_server_id"
                echo -e "${GREEN}服务器ID已更新${NC}"
            fi
            ;;
        3)
            read -p "请输入新的Worker URL: " new_worker_url
            if [ -n "$new_worker_url" ]; then
                WORKER_URL="$new_worker_url"
                echo -e "${GREEN}Worker URL已更新${NC}"
            fi
            ;;
        4)
            return
    ...
    # 重启服务
    echo -e "${YELLOW}配置已更新，是否重启服务应用新配置？(y/n): ${NC}"
